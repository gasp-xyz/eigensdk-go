...
var (
	TaskAlreadyInitializedErrorFn = func(taskIndex types.TaskIndex) error {
		return fmt.Errorf("task %x already initialized", taskIndex)
	}
	...
	OperatorNotPartOfTaskQuorumErrorFn = func(operatorId types.OperatorId, taskIndex types.TaskIndex) error {
		return fmt.Errorf("operator %x not part of task %x's quorum", operatorId, taskIndex)
	}
...
func (a *BlsAggregatorService) InitializeNewTask(
	...
	a.logger.Debug("AggregatorService initializing new task", "taskIndex", taskIndex, "taskReferenceBlock", taskReferenceBlock, "quorumNumbers", quorumNumbers, "quorumThresholdPercentages", quorumThresholdPercentages, "timeToExpiry", timeToExpiry)
	...
	go a.singleTaskAggregatorGoroutineFunc(taskIndex, taskReferenceBlock, quorumNumbers, quorumThresholdPercentages, timeToExpiry, signedTaskRespsC)
	return nil
}
...
func (a *BlsAggregatorService) singleTaskAggregatorGoroutineFunc(
	...
	for {
		...
		case signedTaskResponseDigest := <-signedTaskRespsC:
			...
			if signed { 
				a.logger.Warnf("Operator %#v already submitted response for task %x with same digest. Skipping message.", signedTaskResponseDigest.OperatorId, taskIndex)
				continue
			}
		...
		case <-taskExpiredTimer.C:
			...
			blsAggregationServiceResponse, err := a.createResponse(taskIndex, taskReferenceBlock, quorumNumbers, signedTaskResponseDigest, operatorsAvsStateDict, digestAggregatedOperators, quorumApksG1)
			if err != nil {
				a.aggregatedResponsesC <- BlsAggregationServiceResponse{
					Err: err,
				}
...
func (a *BlsAggregatorService) finishTask(
	...
	for digest, aggregatedOperators := range aggregatedOperatorsDict {
		if checkIfStakeThresholdsMet(a.logger, aggregatedOperators.signersTotalStakePerQuorum, totalStakePerQuorum, quorumThresholdPercentagesMap) {
			blsAggregationServiceResponse, err := a.createResponse(taskIndex, taskReferenceBlock, quorumNumbers, digest, operatorsAvsStateDict, aggregatedOperators, quorumApksG1)
			if err != nil {
				a.aggregatedResponsesC <- BlsAggregationServiceResponse{
					Err: err,
				}
